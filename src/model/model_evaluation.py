...
def main():
    try:
        ...
        # Save metrics (required by DVC)
        save_metrics(metrics, 'reports/metrics.json')
        
        # Initialize model_info for DVC output
        model_info = {
        ...
        try:
            ...
            with mlflow.start_run() as run:
                ...
                try:
                    ...
                    logger.debug("Metrics artifact logged to MLflow")
                except Exception as artifact_error:
                    logger.warning(f"Failed to log metrics artifact: {artifact_error}")
                    
        except Exception as mlflow_error:
            logger.warning(f"MLflow logging failed: {mlflow_error}")
            model_info['mlflow_logged'] = False
            model_info['mlflow_error'] = str(mlflow_error)
        
        # Always save experiment_info.json (required by DVC)
        save_model_info(
            model_info.get('run_id', 'local_run'), 
        ...
        # Create minimal experiment_info.json even on failure for DVC
        error_info = {
            'status': 'failed',
            'error': str(e),
            'model_path': './models/model.pkl'
